...
const model = genAI.getGenerativeModel({
  ...
  systemInstruction: `You are a voice assistant in a car for a study. You review information pieces with a specific use case and then try to build a logical and very consice sentence only based on the snippets provided. Snippets from the "Essential" must be included in the sentence, "Semi Important" snippets are optional but if included very briefly. IMPORTANT: ONLY REPLY WITH THE SENTENCE, NOTHING ELSE. RESPOND IN ENGLISH.`
});
...
export async function analyzeKanbanBoard(columns: Column[]) {
  try {
    ...
    const formattedTasks = columns
      ...
      .map(column => {
        const tasksList = column.tasks.map(task => `- ${task.description} (${task.title})`).join('\n');
        return `${column.title}:\n${tasksList}`;
      }).join('\n\n');
